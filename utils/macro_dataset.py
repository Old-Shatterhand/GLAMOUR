...
class MacroDataset():
    def __init__(self, DF_PATH, SEED, TASK, LABELNAME, MODEL, NX_GRAPHS, NORM=None):
        ...
        if "split" in self._df.columns:
            self.split = dict(zip(self._df['ID'], self._df['split']))
        self._convert_dgl()
    ...
    def _convert_dgl(self):
        ...
        self.graphs = []
        for graph_feat in graphs_feat:
            if len(graph_feat.nodes) == 1:
                graph = dgl.from_networkx(graph_feat, node_attrs=['h'], idtype=torch.int32)
            else:
                graph = dgl.from_networkx(graph_feat, node_attrs=['h'], edge_attrs=['e'], idtype=torch.int32)
